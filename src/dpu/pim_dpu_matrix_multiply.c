...
/**
 * @brief DPU arguments structure for matrix multiplication
 */
typedef struct {
    ...
    uint32_t matrix1_rows;            ///< Rows in first matrix
    uint32_t matrix1_cols;            ///< Columns in first matrix
    uint32_t matrix2_rows;            ///< Rows in second matrix
    uint32_t matrix2_cols;            ///< Columns in second matrix
    uint32_t result_rows;             ///< Rows in result matrix
    uint32_t result_cols;             ///< Columns in result matrix
    uint32_t matrix1_type_size;       ///< Size of first matrix elements (1 for int8_t)
    uint32_t matrix2_type_size;       ///< Size of second matrix elements (1 for int8_t)
    uint32_t result_type_size;        ///< Size of result matrix elements (2 for uint16_t)
} matrix_multiply_arguments_t;

__host matrix_multiply_arguments_t MATRIX_MULTIPLY_ARGUMENTS;
